...
function mod:OnEnable()
    ...
    self:RegisterEvent("CHAT_MSG_CHANNEL", "HandleChatChannel")
    ...
    if self.db.profile.noRealNames then
        ...
        for i = 1, n do
            ...
            local gameAccount = account.gameAccountInfo
            if gameAccount and gameAccount.gameAccountID then
                storedName[gameAccount.gameAccountID] = gameAccount.characterName
            end
        end
    end
...
function mod:AddPlayer(name, class, level, save)
    ...
    if type(name) == "string" and name ~= "" and class and class ~= UNKNOWN then
        local safeName = Ambiguate(name, "none")
        if save or self.db.realm.names[safeName] then
            self.db.realm.names[safeName] = self.db.realm.names[safeName] or {}
            if level and level ~= 0 then
                self.db.realm.names[safeName].level = level
            end
        else
            local_names[safeName] = local_names[safeName] or {}
            local_names[safeName].class = class
            if level and level ~= 0 then
                local_names[safeName].level = level
            end
        end
        cache[safeName] = nil
    end
...
function mod:FRIENDLIST_UPDATE(evt)
    for i = 1, GetNumFriends() do
        ...
        if type(name) == "string" and name ~= "" and class then
            local safeName = Ambiguate(name, "none")
            self:AddPlayer(safeName, localizedToSystemClass[class], level, self.db.profile.saveFriends)
        end
    end
end
...
function mod:GUILD_ROSTER_UPDATE(evt)
    ...
    for i = 1, GetNumGuildMembers() do
        ...
        if type(name) == "string" and name ~= "" then
            local safeName = Ambiguate(name, "none")
            if online then
                channels.GUILD[safeName] = safeName
            end
            self:AddPlayer(safeName, class, level, self.db.profile.saveGuild)
        end
    end
end
...
function mod:GROUP_ROSTER_UPDATE(evt)
    ...
    if IsInRaid() then
        for i = 1, GetNumGroupMembers() do
            ...
            if type(n) == "string" and n ~= "" and c and l then
                local safeName = Ambiguate(n, "none")
                channels.RAID[safeName] = true
                self:AddPlayer(safeName, c, l, self.db.profile.saveParty)
            end
        end
...
function mod:PLAYER_TARGET_CHANGED(evt)
    ...
    if type(n) == "string" and n ~= "" then
        local safeName = Ambiguate(n, "none")
        self:AddPlayer(safeName, c, l, self.db.profile.saveTarget)
    end
end
...
function mod:WHO_LIST_UPDATE(evt)
    if C_FriendList.GetNumWhoResults() <= 3 or self.db.profile.saveAllWho then
        for i = 1, C_FriendList.GetNumWhoResults() do
            ...
            if type(name) == "string" and name ~= "" and class then
                local safeName = Ambiguate(name, "none")
                self:AddPlayer(safeName, class, level, self.db.profile.saveWho)
            end
        end
    end
end

function mod:HandleChatChannel(evt, msg, sender, _, _, _, _, _, _, chan)
    local chanlower = strlower(chan)
    channels[chanlower] = channels[chanlower] or {}
    if sender then
        local safeName = Ambiguate(sender, "none")
        if safeName and safeName ~= "" then
            channels[chanlower][safeName] = true
        end
    end
end
...
function mod:CHAT_MSG_CHANNEL_LEAVE(evt, _, name, _, _, _, _, _, _, chan)
    ...
    if name then
        local safeName = Ambiguate(name, "none")
        if safeName and safeName ~= "" then
            channels[chanlower][safeName] = nil
        end
    end
end
...
local function changeName(msgHeader, name, extra, msgCnt, displayName, msgBody)
    if name ~= player then
        if emphasizeSelfInText then
            msgBody = gsub(gsub(msgBody, "("..player..")", "|cffffff00>|r%1|cffffff00<|r"),
                           "("..player:lower()..")", "|cffffff00>|r%1|cffffff00<|r")
        end
        if colorSelfInText then
            msgBody = gsub(gsub(msgBody, "("..player..")", "|cffff0000%1|r"),
                           "("..player:lower()..")", "|cffff0000%1|r")
        end
    end

    local level
    local safeName = Ambiguate(name, "none")
	local tab = mod.db.realm.names[safeName] or local_names[safeName]
    ...
    return format("|Hplayer:%s%s%s|h%s%s%s|h%s", safeName, extra, msgCnt, leftBracket, displayName, rightBracket, msgBody)
end
    ...
    if not options then
        options = {
            save = {
                ...
                args = {
                    guild = {
                        ...
                        get = function() return mod.db.profile.saveGuild end,
                        set = function(info, v)
                    ...
                    group = {
                        ...
                        get = function() return mod.db.profile.saveGroup end,
                        set = function(info, v)
                    ...
                    friend = {
                        ...
                        get = function() return mod.db.profile.saveFriends end,
                        set = function(info, v)
                    ...
                    target = {
                        ...
                        get = function() return mod.db.profile.saveTarget end,
                        set = function(info, v)
                    ...
                    who = {
                        ...
                        order = 104,
                        get = function() return mod.db.profile.saveWho end,
                        set = function(info, v)
                    ...
                    saveAllWho = {
                        ...
                        order = 105,
                        get = function() return mod.db.profile.saveAllWho end,
                        set = function(info, v) mod.db.profile.saveAllWho = v end
                    },
                    resetDB = {
                        type = "execute",
                        name = L["Reset Data"],
            ...
            bnetBrackets = {
                ...
                set = function(info, v) mod.db.profile.bnetBrackets = v end,
            },
            bnetRealNames = {
                type = "toggle",
                name = L["No RealNames"],
                ...
                set = function(info, v) mod.db.profile.noRealNames = v end,
            },
            useTabComplete = {
                type = "toggle",
            ...
            colorLevelByDifficulty = {
                ...
                order = 105,
                get = function() return mod.db.profile.levelByDiff end,
                set = function(info, v)
