...
local SetMainButtonMixin = CreateFromMixins(UnitPopupButtonBaseMixin);

--function SetMainButtonMixin:GetText()
--	return L["Set Main"]
--end

function SetMainButtonClick(alt)
...
local GetPlayerMenuButtons = UnitPopupMenuPlayer.GetMenuButtons
function UnitPopupMenuPlayer:GetMenuButtons()
	local buttons = GetPlayerMenuButtons(self)
	table.insert(buttons, 1, SetMainButtonMixin)
	return buttons
end

local GetSelfMenuButtons = UnitPopupMenuSelf.GetMenuButtons
function UnitPopupMenuSelf:GetMenuButtons()
	local buttons = GetSelfMenuButtons(self)
	table.insert(buttons, 1, SetMainButtonMixin)
	return buttons
end
...
local GetPartyMenuButtons = UnitPopupMenuParty.GetMenuButtons
function UnitPopupMenuParty:GetMenuButtons()
	local buttons = GetPartyMenuButtons(self)
	table.insert(buttons, 1, SetMainButtonMixin)
	return buttons
end
...
--		rootDescription:CreateTitle("Chat Options")
		rootDescription:CreateButton("Set Player's Main", function() SetMainButtonClick(contextData.name) end) 
	end)
...
-- if this is called it will break in 11.0, but I've never seen it called and don't know what it's here for
function mod.GetMainName()
	print("Debugging: called GetMainName")
...
local function pName(msg, name)
	...
	-- remove the server name from player's name first
	if name:find("-") then 
		name = name:sub(0, name:find("-")-1)
	end
