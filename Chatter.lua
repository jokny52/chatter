...
local proto = {
	...
	TempChatFrames = {},
	AddTempChat = function(self,name) 
		table.insert(self.TempChatFrames,name) 
	end,
...
-- 只在非戰鬥環境下才掛鉤，避免觸發受保護 API
if not InCombatLockdown() then
    if FCF_Tab_OnClick and not self:IsHooked("FCF_Tab_OnClick") then
        self:RawHook("FCF_Tab_OnClick", true)
    end
    if FCF_OpenTemporaryWindow and not self:IsHooked("FCF_OpenTemporaryWindow") then
	...
	-- un clamp chat frame from screen so you can move it in edit mode.
	-- might want to put this elsewhere, or hook to something, if the game re-clamps it under 
	-- some circumstances?
...
do
    ...
    if not InCombatLockdown() then
        ...
        -- 戰鬥中直接呼叫原始 Blizzard 函式，避免插件干涉
        return FCF_Tab_OnClick(...)
    end
...
function Chatter:FCF_OpenTemporaryWindow(chatType, chatTarget, sourceChatFrame, selectWindow)
	local frame
if not InCombatLockdown() then
    frame = self.hooks.FCF_OpenTemporaryWindow(chatType, chatTarget, sourceChatFrame, selectWindow)
else
    -- 戰鬥中直接呼叫原始 Blizzard 函式
    frame = FCF_OpenTemporaryWindow(chatType, chatTarget, sourceChatFrame, selectWindow)
end

	if frame then
		for k, v in self:IterateModules() do
			--if not frame.isDecorated then
			if not frame.isAdded then
