...
local function _RefreshDialogAnchors()
    for index = 1, #active_dialogs do
        ...
        if index == 1 then
            local default_dialog = _G.StaticPopup_FindVisible("ANY")
...
local function EditBox_OnEnterPressed(editbox)
    if not editbox.autoCompleteParams or not (_G.AutoCompleteEditBox_OnEnterPressed and _G.AutoCompleteEditBox_OnEnterPressed(editbox)) then
...
local function EditBox_OnTextChanged(editbox, user_input)
    if not editbox.autoCompleteParams or not (_G.AutoCompleteEditBox_OnTextChanged and _G.AutoCompleteEditBox_OnTextChanged(editbox, user_input)) then
        local dialog = editbox:GetParent()
